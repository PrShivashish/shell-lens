...
from shellens.agents.intent.agent import IntentAgent
from shellens.agents.planner.agent import PlannerAgent
from shellens.agents.execution.agent import ExecutionAgent
from shellens.agents.reasoning.agent import ReasoningAgent
from shellens.agents.response.agent import ResponseAgent
from shellens.agents.profiler.agent import ProfilerAgent
from shellens.agents.visualization.agent import VisualizationAgent
from shellens.agents.reflection.agent import ReflectionAgent
from shellens.agents.goal.agent import GoalAgent

from shellens.execution.executor import Executor
...
from shellens.execution.data_store import get_data, set_data
...
# PROFILER NODE
# -------------------------
def profiler_node(state: ShellLensState):
...
# -------------------------
def planning_node(state: ShellLensState):
    ...
    prompt = f"""
    You are the central brain of Shell Lens AI.
    User Query: {query}
    ...
    from shellens.llm.abstraction import chat
    result = chat(prompt)
...
# -------------------------
def execution_node(state: ShellLensState):
    import hashlib
    ...
    try:
        if res and res.get("type") == "tool":
            from shellens.execution.tools.registry import TOOLS
...
# -------------------------
def response_node(state: ShellLensState):
