from shellens.execution.tools.base import BaseTool
import os
import pandas as pd

def _save_to_source(df):
    try:
        source_path_file = os.path.join(".shellens", "source_path.txt")
        if os.path.exists(source_path_file):
            ...
            return "\nWarning: Could not save to original file! You must run 'shellens load-file <path>' to enable auto-saving for this session."
